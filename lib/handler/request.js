...
import * as vorlonDelegate from '../vorlon'
...
export function httpRequestHandler(req, res) {
    ...
    if (config.enableRule) {
        let activeRuleList = Object.keys(config.ruleList).filter(v => config.ruleList[v].checked)
...
export function connectReqHandler(req, cltSocket, head) {
    let hostname
    ...
    if (config.interceptHttps) {
        getPort(p => {
            ...
            let srvSocket = net.connect(port, hostname, function () {
                cltSocket.write('HTTP/' + req.httpVersion
