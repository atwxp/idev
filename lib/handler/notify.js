...
function getCls(statusCode, contentType) {
    ...
    let keys = Object.keys(contentTypePattern);
    for (let i = 0, l = keys.length; i < l; i++) {
        if (contentTypePattern[keys[i]].test(contentType)) {
            cls.push(keys[i]);
            break;
        }
    }

    keys = Object.keys(statusCodePattern);
    for (let i = 0, l = keys.length; i < l; i++) {
        if (statusCodePattern[keys[i]].test(statusCode)) {
            cls.push(keys[i]);
            break;
        }
    }
