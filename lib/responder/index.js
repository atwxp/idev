...
export default function (rule, req, res) {
    ...
    let fullUrl = ''
        + ((hasProtocol(u.protocol) && u.protocol.slice(0, -1).toLowerCase()) || 'https')
        + '://' + req.headers.host + u.path

    // http://stackoverflow.com/questions/3446170/escape-string-for-use-in-javascript-regex
    let match = pattern.match(/^\s*regexp\s*:\s*(.*)/)

    match = !match ? pattern.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, '\\$&') : match[1].trim()

    pattern = new RegExp(/^https?:/.test(match) ? '^' + match : '^https?://' + match)
    ...
    if (typeof responder === 'string') {
        ...
        if (IPREG.test(responder) || URLREG.test(responder)) {
            ...
            fullUrl = fullUrl.replace(pattern, function (match) {
